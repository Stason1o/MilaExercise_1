...
public class EmployeeService {
    ...
    public Set<Worker> workersReadyForTrip(Map<LocalDateTime, Worker> linkedHashMap, LocalDateTime startDate, LocalDateTime endDate) {
        employeeSet = new LinkedHashSet<>();
        for (LocalDateTime mapIterator : linkedHashMap.keySet()) {
            if (mapIterator.isBefore(endDate) && mapIterator.isAfter(startDate)) {
                employeeSet.add(linkedHashMap.get(mapIterator));
            }
        }
        return employeeSet;
    }
    ...
    public Capability possibilityToVisitCountry(Worker worker, List<String> countries) {
        for (String countryIterator : countries) {
            for (String workerCountries : worker.getAllowedCountries()) {
                if (workerCountries.equalsIgnoreCase(countryIterator)) {
    ...
    public Map<String, List<Worker>> createFinalCollectionToStoreCountryAndWorkers(String country, final List<Worker> workers) {
        final Map<String, List<Worker>> finalMap = new LinkedHashMap<String, List<Worker>>() {
            {
                put(country, workers);
            }
        };
